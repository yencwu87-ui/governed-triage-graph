...
def client(api_key: Optional[str] = None) -> Anthropic:
    """Cached client. Rebuilt whenever the key changes — otherwise a user who
    corrects a mistyped key would keep hitting the old one."""
    global _client, _client_key
    k = api_key or os.environ.get("ANTHROPIC_API_KEY")
    if not k:
        raise RuntimeError("No API key. Set ANTHROPIC_API_KEY or pass api_key=")
    if _client is None or _client_key != k:
        _client = Anthropic(api_key=k)
        _client_key = k
    return _client
...
def classify(subject, body, dq=None, precedents=None, version="v0.1",
             model=MODEL, log_path="calls.jsonl", api_key=None,
             rubric_text=None) -> dict:
    ...
    resp = client(api_key).messages.create(
        ...
        system=SYSTEM.format(rubric=rubric_text or rubric()),
        messages=[{"role": "user",
